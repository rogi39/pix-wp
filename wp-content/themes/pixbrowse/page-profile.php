...
$account_code = get_user_meta($current_user->ID, 'account_code', true);
		...
		<div class="container">
			<?php if (checkConfirmAcc() == false) { ?>
				<div class="profile__form-block">
					...
					<form class="profile__form send-form" action="enter_confirm_code">
						<div class="profile__form-row">
							<div class="profile__form-col">
								<input type="text" class="profile__form-input" placeholder="Enter code" name="confirm_code">
							</div>
						</div>
						<input type="hidden" name="user_id" value="<?php echo $current_user->ID; ?>">
						<?php wp_nonce_field('enter_confirm_code_nonce_action', 'enter_confirm_code_nonce_field'); ?>
						<button class="profile__form-btn">Confirm</button>
					</form>
				</div>
			<?php } else { ?>
